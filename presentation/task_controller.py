...
@router.put('/{id}/done')
def do_task(id: str, repo: TaskRepository = Depends(TaskRepository), logged_user: UserRead = Depends(get_logged_user)):
    task = repo.get_by_id(id)
    
    if task is not None and task.user_id == logged_user.id:
        repo.mark_status(task, True)
    else:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)

@router.put('/{id}/undone')
def undone_task(id: str, repo: TaskRepository = Depends(TaskRepository), logged_user: UserRead = Depends(get_logged_user)):
    task = repo.get_by_id(id)
    
    if task is not None and task.user_id == logged_user.id:
        repo.mark_status(task, False)
    else:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
